...
def on_callback(call):
    ...
    try:
        ...
        if data.startswith("mj_"):   # mafia join
            ...
            count = len(g['players'])
            note = " (нужно ещё минимум {})".format(max(0, 4 - count)) if count < 4 else ""
            ai_tip = "\n💡 Мало игроков? Нажми «🤖 Добавить ИИ» в меню игр!" if count < 4 else ""
            send_group(cid, f"✅ {uname} присоединился к мафии! Игроков: {count}{note}{ai_tip}")
            return
...
def _group_mafia_count_votes(chat_id):
    ...
    def _resolve(_cid=chat_id, _count=dict(count)):
        ...
        if not _count:
            host = ask_ai(
                "Ты ведущий мафии в группе. Никто не проголосовал. "
                "Объяви что день продолжается — коротко, жутко. 1 предложение.",
                chat_id=_cid, dm_mode=True
            )
        ...
        if len(top) > 1:
            ...
            host = ask_ai(
                f"Ты ведущий мафии. Ничья в голосовании между: {names}. "
                "День продолжается. 1 предложение напряжённо.",
                chat_id=_cid, dm_mode=True
            )
            g2["votes"] = {}; g2["voted_today"] = set()
        ...
        host = ask_ai(
            f"Ты ведущий мафии в группе. Игрок '{elim_name}' устранён голосованием. "
            f"Объяви это жутко — НЕ НАЗЫВАЙ его роль, это сюрприз. 1-2 предложения.",
            chat_id=_cid, dm_mode=True
        )
        ...
        night_host = ask_ai(
            "Ты ведущий мафии. Объяви наступление ночи жутко. 1 предложение.",
            chat_id=_cid, dm_mode=True
        )
        send_group(_cid,
            f"🌙 НОЧЬ {g2['day_num']}\n\n"
        ...
        if tl.startswith("/ai ") or tl.startswith("спроси ии "):
            ...
            if q:
                def _dm_ai(_q=q, _uid=uid, _u=u):
                    answer = ask_ai(f"Жертва пишет мне: '{_q}'", chat_id=_uid, dm_mode=True)
                    send(_uid, f"👁 {answer}", kb=KB(_u.get("stage", 0)))
                _pool.submit(_dm_ai)
                return
        # Случайный жуткий комментарий (stage >= 2, 7% шанс)
        if stage >= 2 and random.random() < 0.07 and len(text) > 8 and not text.startswith("/"):
            def _dm_creep(_text=text, _uid=uid, _u=u):
                time.sleep(random.uniform(3, 8))
                answer = ask_ai(
                    f"Жертва написала в боте: '{_text[:80]}'. "
                    f"Вмешайся жутко, намекни что следишь. 1-2 предложения.",
                    chat_id=_uid, dm_mode=True
                )
                if answer:
                    send(_uid, f"👁 {answer}", kb=KB(_u.get("stage", 0)))
            _pool.submit(_dm_creep)
...
# Имена для ИИ-ботов в DM мафии
_AI_BOT_NAMES = [
    "Алекс", "Борис", "Виктор", "Григорий", "Денис",
    "Елена", "Жанна", "Зоя", "Ирина", "Кирилл",
    "Лиза", "Максим", "Нина", "Олег", "Павел",
    "Рита", "Сергей", "Таня", "Ульяна", "Фёдор"
]
_AI_BOT_ID_START = -1000  # отрицательные ID для ИИ-ботов

def _add_ai_bots_to_lobby(g, lobby_id, count_needed):
    """Добавляет ИИ-ботов в лобби чтобы набрать count_needed игроков."""
    used_names = set(g["player_names"].values())
    available = [n for n in _AI_BOT_NAMES if n not in used_names]
    random.shuffle(available)
    added = []
    bot_id = _AI_BOT_ID_START - len([p for p in g["players"] if p <= _AI_BOT_ID_START])
    for i in range(count_needed):
        name = available[i % len(available)] if available else f"Гость{i+1}"
        g["players"].append(bot_id)
        g["player_names"][bot_id] = name
        g.setdefault("ai_bots", set()).add(bot_id)
        added.append((bot_id, name))
        bot_id -= 1
    return added

def start_mafia_dm(lobby_id):
    """Запускает игру в мафию через ЛС. v19: ИИ-боты заполняют до 4."""
    g = _mafia_lobby.get(lobby_id)
    if not g:
        return
    players = g["players"]
    real_players = [p for p in players if p > 0]

    # Автодобавление ИИ-ботов если мало игроков
    ai_note = ""
    if len(players) < 4:
        needed = 4 - len(players)
        added = _add_ai_bots_to_lobby(g, lobby_id, needed)
        ai_note = f"\n\n🤖 Добавлено {needed} ИИ-игрок(а): {', '.join(n for _,n in added)}"
        players = g["players"]
    ...
    g["phase"] = "day"
    g["day_num"] = 1
    g.setdefault("ai_bots", set())
    g["ai_messages"] = []  # очередь фейковых сообщений от ИИ

    # Рассылаем роли живым игрокам
    for uid in real_players:
        role = roles.get(uid, "мирный")
        send(uid,
            f"🎭 МАФИЯ НАЧИНАЕТСЯ!{ai_note}\n\n"
            f"Игроков: {len(players)} (из них {len(g.get('ai_bots',set()))} ИИ)\n\n"
            f"{MAFIA_ROLES[role]}"
        )

    # ИИ-ведущий объявляет старт
    def _host_intro(_lid=lobby_id):
        time.sleep(1.5)
        g2 = _mafia_lobby.get(_lid)
        if not g2: return
        names = list(g2["player_names"].values())
        intro = ask_ai(
            f"Ты ведущий игры в мафию. Игроки: {', '.join(names)}. "
            f"Объяви старт игры атмосферно и жутко. 2-3 предложения.",
            chat_id=_lid, dm_mode=True
        )
        for uid in [p for p in g2["alive"] if p > 0]:
            try: send(uid, f"🎭 Ведущий: {intro}")
            except: pass
    _pool.submit(_host_intro)
...
def _mafia_announce_day(lobby_id):
    ...
    if not g:
        return
    ai_bots = g.get("ai_bots", set())
    alive_names = "\n".join(
        f"  {'🤖' if p in ai_bots else '👤'} {g['player_names'].get(p,'?')}"
        for p in g["alive"]
    )
    kb_day = InlineKeyboardMarkup(row_width=1)
    for p in g["alive"]:
        pname = g["player_names"].get(p, "?")
        icon = "🤖" if p in ai_bots else "👤"
        kb_day.add(InlineKeyboardButton(
            f"⚖️ Устранить: {icon} {pname}",
    ...
    def _day_announce(_lid=lobby_id, _names=alive_names):
        g2 = _mafia_lobby.get(_lid)
        if not g2: return
        host_msg = ask_ai(
            f"Ты ведущий мафии. День {g2['day_num']}. "
            f"Живые игроки: {', '.join(g2['player_names'].get(p,'?') for p in g2['alive'])}. "
            f"Объяви начало дня и попроси обсудить кто мафия. 1-2 предложения жутко.",
            chat_id=_lid, dm_mode=True
        )
        day_text = (
            f"☀️ ДЕНЬ {g2['day_num']}\n\n"
            f"🎭 Ведущий: {host_msg}\n\n"
            f"Живые игроки:\n{_names}\n\n"
            f"Выбери кого хочешь устранить:"
        )
        for uid in [p for p in g2["alive"] if p not in ai_bots]:
            try: bot.send_message(uid, day_text, reply_markup=kb_day)
            except: pass

        # Фейковые сообщения от ИИ-ботов (обсуждение)
        time.sleep(random.uniform(2, 5))
        real_alive = [p for p in g2["alive"] if p not in ai_bots]
        for bot_uid in ai_bots:
            if bot_uid not in g2["alive"]: continue
            bot_name = g2["player_names"].get(bot_uid, "?")
            comment = ask_ai(
                f"Ты игрок '{bot_name}' в мафии. День {g2['day_num']}. "
                f"Напиши что думаешь о ситуации — кратко, как реальный игрок. 1 фраза.",
                chat_id=_lid
            )
            for uid in [p for p in g2["alive"] if p not in ai_bots]:
                try: send(uid, f"💬 {bot_name}: {comment}")
                except: pass
            time.sleep(random.uniform(1.5, 3))

        # ИИ-боты голосуют автоматически через задержку
        time.sleep(random.uniform(4, 8))
        g3 = _mafia_lobby.get(_lid)
        if not g3 or g3.get("phase") != "day": return
        for bot_uid in list(ai_bots):
            if bot_uid not in g3.get("alive", []): continue
            if bot_uid in g3.get("votes", {}): continue
            targets = [p for p in g3["alive"] if p != bot_uid]
            if not targets: continue
            # ИИ-мафия старается убрать мирного, ИИ-мирный — голосует по подозрению
            role = g3["roles"].get(bot_uid, "мирный")
            if role == "мафия":
                peaceful = [p for p in targets if g3["roles"].get(p) != "мафия"]
                target = random.choice(peaceful) if peaceful else random.choice(targets)
            else:
                target = random.choice(targets)
            g3["votes"][bot_uid] = target
            _check_mafia_day_votes(_lid)

    _pool.submit(_day_announce)
...
def _mafia_announce_night(lobby_id):
    ...
    # ИИ-ведущий объявляет ночь
    def _night_host(_lid=lobby_id):
        g2 = _mafia_lobby.get(_lid)
        if not g2: return
        host_msg = ask_ai(
            f"Ты ведущий мафии. Объяви наступление ночи жутко и атмосферно. "
            f"Живые: {len(g2['alive'])} игроков. 1-2 предложения.",
            chat_id=_lid, dm_mode=True
        )
        for uid in [p for p in g2["alive"] if p not in ai_bots]:
            role = g2["roles"].get(uid, "мирный")
            if role == "мирный":
                try: send(uid, f"🌙 НОЧЬ {g2['day_num']}\n\n🎭 Ведущий: {host_msg}\n\nТы мирный. Жди до утра...")
                except: pass
            elif role in ("мафия", "шериф", "доктор"):
                targets = [p for p in g2["alive"] if (role != "доктор" and p != uid) or role == "доктор"]
                ...
                for p in targets:
                    pname = g2["player_names"].get(p, "?")
                    icon = "🤖" if p in ai_bots else "👤"
                    kb_night.add(InlineKeyboardButton(
                        f"🎯 {action_label}: {icon} {pname}",
                        callback_data=f"mdm_{_lid}_{uid}_{p}"
                    ))
                try:
                    bot.send_message(uid,
                        f"🌙 НОЧЬ {g2['day_num']} — ТЫ {role.upper()}\n\n"
                        f"🎭 Ведущий: {host_msg}\n\n"
                        f"Выбери {action_desc}:",
                        reply_markup=kb_night)
                except: pass
        ...
        time.sleep(random.uniform(3, 8))
        g3 = _mafia_lobby.get(_lid)
        if not g3 or g3.get("phase") != "night": return
        for bot_uid in list(ai_bots):
            if bot_uid not in g3.get("alive", []): continue
            if bot_uid in g3.get("night_actions", {}): continue
            role = g3["roles"].get(bot_uid, "мирный")
            if role == "мирный":
                continue  # мирные не действуют
            if role == "мафия":
                targets = [p for p in g3["alive"] if p != bot_uid and g3["roles"].get(p) != "мафия"]
            elif role == "шериф":
                targets = [p for p in g3["alive"] if p != bot_uid]
            elif role == "доктор":
                targets = list(g3["alive"])
            else:
                continue
            if not targets: continue
            target = random.choice(targets)
            g3["night_actions"][bot_uid] = target
            if role == "шериф":
                # ИИ-шериф получает результат — пока не объявляет (он бот)
                pass
            _check_mafia_night_actions(_lid)

    _pool.submit(_night_host)
...
def _check_mafia_day_votes(lobby_id):
    """Проверяет набрали ли все живые голоса. v19: ИИ-ведущий + боты."""
    g = _mafia_lobby.get(lobby_id)
    if not g:
        return
    ai_bots = g.get("ai_bots", set())
    ...
    alive = set(g["alive"])
    # Ждём только реальных игроков (ИИ-боты голосуют сами)
    real_alive = alive - ai_bots
    if not real_alive.issubset(voted):
        return  # ждём живых игроков
    ...
    def _announce(_lid=lobby_id, _count=dict(count)):
        g2 = _mafia_lobby.get(_lid)
        if not g2: return
        ai_bots2 = g2.get("ai_bots", set())
        if not _count:
            host_msg = ask_ai(
                "Ты ведущий мафии. Голосование закончилось — никто не проголосовал. "
                "Объяви это жутко. 1 предложение.",
                chat_id=_lid, dm_mode=True
            )
            for uid in [p for p in g2["alive"] if p not in ai_bots2]:
                send(uid, f"🎭 Ведущий: {host_msg}\n\nНикого не устранили.")
        else:
            eliminated = max(_count, key=_count.get)
            elim_name = g2["player_names"].get(eliminated, "?")
            elim_role = g2["roles"].get(eliminated, "мирный")
            if eliminated in g2["alive"]:
                g2["alive"].remove(eliminated)
            host_msg = ask_ai(
                f"Ты ведущий мафии. Игрок '{elim_name}' устранён голосованием. "
                f"Объяви это атмосферно — НЕ РАСКРЫВАЙ его роль. 1-2 предложения.",
                chat_id=_lid, dm_mode=True
            )
            role_reveal = MAFIA_ROLES.get(elim_role, "?").split("\n")[0]
            for uid in [p for p in (g2["alive"] + [eliminated]) if p not in ai_bots2]:
                try:
                    send(uid,
                        f"🎭 Ведущий: {host_msg}\n\n"
                        f"⚖️ Устранён: {elim_name}\n"
                        f"Его роль была: {role_reveal}"
                    )
                except: pass

        g2["votes"] = {}
        winner = _check_mafia_dm_win(_lid)
        if winner:
            _end_mafia_dm(_lid, winner); return
        g2["day_num"] += 1
        _mafia_announce_night(_lid)

    _pool.submit(_announce)

def _check_mafia_night_actions(lobby_id):
    """Ждёт пока все ночные роли сделают ход. v19: ИИ-ведущий."""
    g = _mafia_lobby.get(lobby_id)
    if not g:
        return
    ai_bots = g.get("ai_bots", set())
    ...
    for uid, target in g["night_actions"].items():
        ...
        if role == "мафия":
            ...
            # Шериф получает результат в ЛС (только реальные игроки)
            if uid not in ai_bots:
                target_role = g["roles"].get(target, "?")
                is_mafia = (target_role == "мафия")
                target_name = g["player_names"].get(target, "?")
                try: send(uid, f"🔎 Твоя проверка: {target_name} — {'🔫 ЭТО МАФИЯ!' if is_mafia else '👤 мирный житель'}")
                except: pass

    def _morning(_lid=lobby_id, _mt=mafia_target, _sd=saved_by_doctor):
        g2 = _mafia_lobby.get(_lid)
        if not g2: return
        ai_bots2 = g2.get("ai_bots", set())
        real_alive = [p for p in g2["alive"] if p not in ai_bots2]

        if _mt and _mt != _sd:
            victim_name = g2["player_names"].get(_mt, "?")
            victim_role = g2["roles"].get(_mt, "мирный")
            g2["alive"] = [p for p in g2["alive"] if p != _mt]
            host_msg = ask_ai(
                f"Ты ведущий мафии. Утро. Ночью был убит игрок '{victim_name}'. "
                f"Объяви это жутко и атмосферно — НЕ НАЗЫВАЙ роль жертвы. 1-2 предложения.",
                chat_id=_lid, dm_mode=True
            )
            role_txt = MAFIA_ROLES.get(victim_role, "?").split("\n")[0]
            for uid in real_alive + ([_mt] if _mt not in ai_bots2 else []):
                try:
                    send(uid,
                        f"☀️ УТРО\n\n🎭 Ведущий: {host_msg}\n\n"
                        f"💀 Погиб: {victim_name}\nРоль была: {role_txt}"
        ...
        elif _mt and _mt == _sd:
            saved_name = g2["player_names"].get(_sd, "?")
            host_msg = ask_ai(
                f"Ты ведущий мафии. Утро. Доктор спас игрока этой ночью. Никто не погиб. "
                f"Объяви это интригующе. 1 предложение.",
                chat_id=_lid, dm_mode=True
            )
            for uid in real_alive:
                try: send(uid, f"☀️ УТРО\n\n🎭 Ведущий: {host_msg}\n\nНикто не погиб.")
                except: pass
        else:
            host_msg = ask_ai(
                "Ты ведущий мафии. Утро. Ночью никто не погиб — мафия промахнулась. "
                "Объяви кратко и жутко. 1 предложение.",
                chat_id=_lid, dm_mode=True
            )
            for uid in real_alive:
                try: send(uid, f"☀️ УТРО\n\n🎭 Ведущий: {host_msg}\n\nНикто не погиб.")
...
def _end_mafia_dm(lobby_id, winner):
    ...
    ai_bots = g.get("ai_bots", set())
    roles_reveal = "\n".join(
        f"  {'🤖' if uid in ai_bots else '👤'} {g['player_names'].get(uid,'?')} — {role}"
        for uid, role in g["roles"].items()
    )
    # ИИ-ведущий финальное слово
    try:
        finale = ask_ai(
            f"Ты ведущий мафии. Игра завершена. Победили {'мирные жители' if winner=='мирные' else 'мафия'}. "
            f"Произнеси финальный монолог — жуткий, атмосферный. 2-3 предложения.",
            chat_id=lobby_id, dm_mode=True
        )
    except:
        finale = "Игра окончена. Город никогда не будет прежним."

    if winner == "мирные":
        result = "🎉 МИРНЫЕ ЖИТЕЛИ ПОБЕДИЛИ!"
    else:
        result = "🔫 МАФИЯ ПОБЕДИЛА!"

    for uid in g["players"]:
        _mafia_player.pop(uid, None)
        if uid in ai_bots: continue
        u = U(uid)
        try:
            send(uid,
                f"{result}\n\n"
