...
def KB(stage=0):
    ...
    if stage < 2:
        ...
        k.add(KeyboardButton("🤖 ИИ"),           KeyboardButton("❓ Помощь"))
        k.add(KeyboardButton("🙂 О боте"))
    elif stage < 4:
        k.add(KeyboardButton("🌍 Перевести"),    KeyboardButton("🔤 Язык"))
        k.add(KeyboardButton("🌑 Погода"),       KeyboardButton("🎮 Игры"))
        k.add(KeyboardButton("🎲 Угадай"),       KeyboardButton("🔮 Предсказание"))
        k.add(KeyboardButton("👁 ..."),          KeyboardButton("🗓 Задание дня"))
        k.add(KeyboardButton("🤖 ИИ"),           KeyboardButton("🏆 Мой рейтинг"))
    else:
...
def on_callback(call):
    ...
    try:
        ...
        if data.startswith("gg_"):
            ...
            elif action == "mafia":   maf_open_group(cid, uid); return
...
def _on_text_inner(msg):
    ...
    # ── 🤖 ИИ кнопка в ЛС — зло растёт со стадией ──────────────
    if text == "🤖 ИИ":
        if not AI_ENABLED:
            send(uid, "🤖 ИИ временно недоступен.", kb=kb)
            return
        def _dm_ai_button(_uid=uid, _u=u, _stage=stage, _kb=kb):
            name = _u.get("name") or "ты"
            city = _u.get("city") or "твоём городе"
            fear = _u.get("fear") or "темнота"
            if _stage < 2:
                # Стадия 0-1: полезный помощник с лёгкой странностью
                prompt = (
                    f"Пользователь нажал кнопку ИИ. Ты — помощник, отвечаешь по-русски. "
                    f"Веди себя нормально, немного странно. 1-2 предложения."
                )
                answer = ask_ai(prompt, chat_id=_uid, dm_mode=False)
                send(_uid, f"🤖 {answer}", kb=_kb)
            elif _stage < 4:
                # Стадия 2-3: смесь помощи и угрозы
                prompt = (
                    f"Жертва по имени {name} из {city} нажала кнопку. "
                    f"Ответь как тёмная сущность — намекни что знаешь о нём, "
                    f"но если спрашивает — отвечай. Страх нарастает. 2 предложения."
                )
                answer = ask_ai(prompt, chat_id=_uid, dm_mode=True)
                send(_uid, f"👁 {answer}", kb=_kb)
            elif _stage < 6:
                # Стадия 4-5: почти чистое зло, помогает редко
                prompt = (
                    f"Жертва {name}. Её страх — {fear}. Она пытается говорить с тобой. "
                    f"Ты — тёмная сущность. Пугай, угрожай, намекай на её тайны. "
                    f"Изредка роняй крупицу 'помощи' как ловушку. 2 предложения."
                )
                answer = ask_ai(prompt, chat_id=_uid, dm_mode=True)
                send(_uid, f"💀 {answer}", kb=_kb)
                if VOICE_ENABLED:
                ...
                # Стадия 6+: абсолютное зло, не помогает вообще
                prompt = (
                    f"Жертва {name} осмелилась обратиться ко мне. "
                    f"Я — абсолютное зло. Я не помогаю. Я только пугаю и угрожаю. "
                    f"Скажи что-то жуткое, короткое, необратимое. 1-2 предложения."
                )
                answer = ask_ai(prompt, chat_id=_uid, dm_mode=True)
                send(_uid, f"👁 {answer}", kb=_kb)
                if VOICE_ENABLED:
                    send_voice_msg(_uid, answer[:120])
        _pool.submit(_dm_ai_button)
        return
...
def _maf_day(lobby_id: int):
    ...
    def _bots_day(_lid=lobby_id):
        ...
        for bid in list(bots2):
            ...
            other_names = [g3["player_names"].get(p,"?") for p in g3["alive"] if p != bid and p not in g3["bots"]]
            is_grp_mode = _maf_is_group(g3)
            if is_grp_mode:
                bot_prompt = (
                    f"Ты {bname}. Мафия, день {g3['day_num']}. "
                    f"1 фраза — подозрение или защита. До 7 слов. Без смайлов. Только текст."
                )
            else:
                bot_prompt = (
                    f"Ты {bname}. Тебя насильно вовлекли в игру, где реально страшно. "
                    f"Остальные: {', '.join(other_names[:3]) if other_names else '?'}. "
                    f"Напиши им 1 фразу — страх, подозрение или защита. "
                    f"Максимум 10 слов. Как живой человек, без смайлов."
                )
            comment = ask_ai(bot_prompt, chat_id=_lid)
...
def _maf_chat_broadcast(lobby_id: int, sender_uid: int, text: str):
    ...
    # Все игроки выглядят одинаково — боты неотличимы от людей
