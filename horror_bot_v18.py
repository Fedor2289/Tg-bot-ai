...
def proc_game(uid, text):
    ...
    # Кнопки главного меню всегда выводят из игры
    if text in _MAIN_BUTTONS:
        del games[uid]; return False  # передаём главному обработчику
...
def _handle_admin_inner(msg, admin_uid):
    ...
    if adm_state.get(admin_uid, {}).get("step") == "wait_ai_intercept_text":
        ...
        if tid and text.strip():
            ...
            # Голосовое сообщение (как настоящий ИИ)
            if VOICE_ENABLED:
...
def on_start(msg):
    ...
    # Немедленно сбрасываем всё состояние (без задержки)
    with _lock:
        if uid in users:
            users[uid]["stopped"] = True
            users[uid]["muted"]   = True
        games.pop(uid, None)
        _card_story.pop(uid, None)
        _maf_uid.pop(uid, None)
        _squad_mode.pop(uid, None)
        _ai_scare_active.pop(uid, None)
        _auto_mode.discard(uid)

    def _do_start(_uid=uid, _uname=uname):
        # Короткая пауза — даём потокам хоррора заметить флаг stopped
        time.sleep(0.3)

        # Полная очистка и сброс профиля
        with _lock:
            if _uid in users:
                del users[_uid]
        ...
        _card_story.pop(_uid, None)       # выход из карточной истории
        _ai_scare_active.pop(_uid, None)  # выключаем ИИ-атаку
        # Выход из мафии если был
        _lid = _maf_uid.pop(_uid, None)
        if _lid:
            _g = _maf.get(_lid)
            if _g:
                _g.get("alive", []) and _uid in _g["alive"] and _g["alive"].remove(_uid)
                _g.get("players", []) and _uid in _g["players"] and _g["players"].remove(_uid)
        ...
        # Создаём свежий профиль (stopped=False по умолчанию)
        u = U(_uid)
        if _uname:
            u["username"] = _uname
        # Гарантируем что stopped=False
        u["stopped"] = False
        u["muted"]   = False
...
def on_callback(call):
    ...
    try:
        ...
        # ── Кнопка "✍️ Ответить за ИИ" — новый intercept ──────
        if data.startswith("ai_ic_"):
            # data = "ai_ic_{key}_{admin_uid}"
            # key = "{uid}_{timestamp}"
            parts = data.split("_")
            # ai_ic_{uid}_{ts}_{aid}
            try:
                aid = int(parts[-1])
            ...
            if ic.get("group"):
                ...
                send(aid, f"✍️ Пиши ответ для группы {grp_cid}. Будет отправлен как 🤖 ИИ:")
            else:
                # Ответ идёт конкретному юзеру
                target_uid = ic.get("uid")
                adm_state[aid] = {"step": "wait_ai_intercept_text", "target_uid": target_uid}
                u_tgt = U(target_uid)
                n_tgt = u_tgt.get("name", str(target_uid))
                send(aid, f"✍️ Пиши ответ для {n_tgt}. Будет отправлен от имени ИИ:")
            return
        ...
        if data.startswith("ai_intercept_write_"):
            ...
            try:
                ...
                bot.answer_callback_query(call.id, "Ошибка"); return
            for k in list(_ai_intercept.keys()):
                if _ai_intercept[k].get("target_uid") == tid or _ai_intercept[k].get("uid") == tid:
                    _ai_intercept[k]["cancelled"] = True
            bot.answer_callback_query(call.id, "✍️ Напиши сообщение")
            adm_ctx_reset(aid)
...
def _on_text_inner(msg):
    ...
    # Admin'ы идут в отдельный обработчик (всегда, до всего остального)
    if is_admin(uid):
    ...
    if u.get("ai_mode") and text and not text.startswith("/"):
        ...
        if text not in ui_buttons:
            ...
            fear  = u.get("fear")  or "темнота"
            question = text.strip()
            def _ai_dialog_or_intercept(_q=question, _uid=uid, _stage=stage, _u=u, _kb=kb):
                """Сначала форвардим админу на перехват. Через 15с — ИИ если не ответил."""
                _name = _u.get("name") or "?"
                _city = _u.get("city") or "?"
                _fear = _u.get("fear") or "темнота"
                _prefix = "🤖" if _stage < 2 else ("👁" if _stage < 4 else "💀")

                # Строим промпт заранее
                if _stage < 2:
                    _prompt = (f"Пользователь спрашивает: '{_q}'. Ты — умный помощник, "
                               f"отвечаешь по-русски. Дай полный полезный ответ. Максимум 3 предложения.")
                elif _stage < 4:
                    _prompt = (f"Жертва {_name} из {_city} спрашивает: '{_q}'. "
                               f"Ты — тёмная сущность. Ответь жутко, но по существу. 2-3 предложения.")
                elif _stage < 6:
                    if random.random() < 0.6:
                        _prompt = (f"Жертва {_name} (страх: {_fear}) спрашивает: '{_q}'. "
                                   f"Ты — абсолютное зло. Ответь искажённо, зловеще. 2 предложения.")
                    else:
                        _prompt = (f"Жертва пытается говорить. Вопрос: '{_q}'. "
                                   f"Ты — зло. Не отвечай на вопрос. Пугай. 1-2 предложения.")
                else:
                    _prompt = (f"Жертва {_name} спросила: '{_q}'. "
                               f"Ты — абсолютное зло. Игнорируй вопрос. Угрожай. 1 предложение.")

                # Отправляем ВСЕМ админам на перехват
                _intercept_key = f"{_uid}_{int(time.time())}"
                _ai_intercept[_intercept_key] = {"cancelled": False, "uid": _uid}
                ...
                for _aid in list(admins):
                    try:
                        _u_t = U(_uid)
                        _ic_text = (
                            f"👤 {_name} (ID:{_uid}, стадия:{_stage}) написал:\n\n"
                            f"💬 «{_q}»\n\n"
                            f"⏱ Нажми кнопку за 15с — ответишь за ИИ.\n"
                            f"Иначе ответит ИИ автоматически."
                        )
                        _kb_ic = InlineKeyboardMarkup()
                        _kb_ic.add(InlineKeyboardButton(
                            "✍️ Ответить за ИИ",
                            callback_data=f"ai_ic_{_intercept_key}_{_aid}"
                        ))
                        bot.send_message(_aid, _ic_text, reply_markup=_kb_ic)
                ...
                # Проверяем не перехватил ли admin
                ic = _ai_intercept.get(_intercept_key, {})
                if ic.get("cancelled"):
                    _ai_intercept.pop(_intercept_key, None)
                    return  # admin ответил сам
            ...
            _pool.submit(_ai_dialog_or_intercept)
            return
...
def group_ai_respond(chat_id: int, prompt: str, sender_name: str = ""):
    """ИИ отвечает в группу. Сначала форвардит админу на перехват (15с)."""
    full_prompt = f"{sender_name}: {prompt}" if sender_name else prompt

    # Форвардим администратору на перехват
    _ic_key = f"grp_{chat_id}_{int(time.time())}"
    _ai_intercept[_ic_key] = {"cancelled": False, "chat_id": chat_id, "group": True}
    ...
    for _aid in list(admins):
        try:
            ...
            bot.send_message(_aid,
                f"👥 Группа {chat_id} — {sender_name} написал:\n\n«{prompt[:200]}»\n\n⏱ 15с — ответишь сам или ИИ ответит.",
                reply_markup=_kb_ic
            )
        except Exception:
            pass
...
def maf_proc_dm(uid: int, text: str) -> bool:
    ...
    # Чат во время игры (не команды кнопки главного меню)
    if g["state"] == "playing" and uid in g["alive"] and not tl.startswith("/"):
        # Главные кнопки меню — позволяем им сработать (выходим из перехвата)
        if text in _MAIN_BUTTONS:
            return False
        # Сбрасываем режим ИИ-диалога пока идёт мафия
        U(uid)["ai_mode"] = False
...
def proc_card_story(uid, text):
    ...
    # Главные кнопки — выходим из истории
    if text in _MAIN_BUTTONS:
        del _card_story[uid]
        return False  # передаём основному обработчику
