...
def _group_mafia_count_votes(chat_id):
    ...
    g["phase"] = "night"
    g["night_actions"] = {}
    night_msg = f"🌙 НОЧЬ {g['day_num']}\n\nВсе закрыли глаза...\nМафия, шериф и доктор — действуйте в личке бота!"
    send_group(chat_id, night_msg)
    if GROUP_AUTO_VOICE:
        _pool.submit(_send_group_voice, chat_id, f"Ночь {g['day_num']}. Все закрыли глаза.")
    for p in g["alive"]:
        if p == AI_PLAYER_ID:
            # ИИ делает ночной ход автоматически
            _pool.submit(_ai_mafia_night_act, chat_id)
            continue
        role = g["roles"].get(p)
