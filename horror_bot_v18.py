...
def _on_text_inner(msg):
    ...
    if u.get("ai_mode") and text and not text.startswith("/"):
        _dm_ai_buttons = {
            ...
            "🔫 Мафия","🔫 Мафия (создать лобби)",
            # Кнопки admin-панели (не уходят в ИИ)
            "👥 Жертвы","📊 Статистика","📊 Полная статистика",
            "💀 Ужас всем","🛑 Стоп всем","▶️ Рестарт всем","🔇 Тишина всем",
            "🔊 Звук всем","📤 Рассылка всем","⚙️ Выбрать жертву","📋 Список ID",
            "💬 Чат 3 мин","💬 Чат 10 мин","🔕 Стоп чат","👥 Чат деанон",
            "🏆 Лидеры страха","🎬 Все сценарии","🗓 Ежедн. задание всем",
            "🎲 Случай. событие","👑 Мои со-admin'ы","➕ Добавить admin'а",
            "➖ Убрать admin'а","👥 Группы (управление)","🚫 Забанить жертву",
            "✅ Разбанить жертву","📡 Отправить по ID","🗑 Сбросить всех",
            "🤖 ИИ: Groq","🤖 ИИ: Cerebras","🤖 ИИ: Авто","🤖 ИИ: Статус",
            "🤖 ИИ пишет за меня","🔙 Выйти из бога",
        }
        if text not in _dm_ai_buttons and AI_ENABLED:
            _stage_ai = u.get("stage", 0)
            _q_ai = text.strip()
            def _admin_ai(_q=_q_ai, _uid=uid, _st=_stage_ai, _kb=KB(_stage_ai)):
                if _st < 2:
                    ans = ask_ai(f"Вопрос: '{_q}'. Ответь полезно, по-русски. 3 предложения.", chat_id=_uid, dm_mode=False)
                    send(_uid, f"🤖 {ans}", kb=_kb)
                else:
                    ans = ask_ai(f"Жертва спрашивает: '{_q}'. Отвечай жутко.", chat_id=_uid, dm_mode=True)
                    send(_uid, f"👁 {ans}", kb=_kb)
            _pool.submit(_admin_ai)
            return
