...
def on_callback(call):
    ...
    try:
        ...
        # ── Кнопка "✍️ Написать самому" (ИИ-перехват) ──────────
        if data.startswith("ai_intercept_write_"):
            parts = data.split("_")
            # ai_intercept_write_{tid}_{aid}
            try:
                tid = int(parts[3])
                aid = int(parts[4])
            except Exception:
                bot.answer_callback_query(call.id, "Ошибка"); return
            # Отмечаем как перехваченное
            for k in list(_ai_intercept.keys()):
                v = _ai_intercept[k]
                if v.get("target_uid") == tid:
                    v["cancelled"] = True
            bot.answer_callback_query(call.id, "✍️ Напиши сообщение — отправим жертве")
            adm_ctx_reset(aid)
