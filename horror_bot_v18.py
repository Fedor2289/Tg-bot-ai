...
def _handle_group_message(msg, uid, chat_id, text):
    ...
    if awaiting and text and not text.startswith("/"):
        ...
        if mode == "weather":
            ...
            _p = text
            def _ask(_p=_p):
                r = ask_ai(f"{uname} спрашивает: {_p}", chat_id=chat_id)
                if r: send_group(chat_id, f"🤖 ИИ: {r}")
            _pool.submit(_ask)
            return
    ...
    if AI_ENABLED:
        ...
        addressed = any(tl.startswith(k) for k in ("ии,","ии!","бот,","бот!","ai,"))
        if is_reply or addressed:
            def _r():
                r = ask_ai(f"{uname}: {text}", chat_id=chat_id)
                if r: send_group(chat_id, f"🤖 ИИ: {r}")
