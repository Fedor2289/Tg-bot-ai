    ...
    try:
        ...
        if data.startswith("maf_join_"):
            ...
            if ok and g:
                ...
                _maf_send_all(lid_i, f"✅ {uname} вступил! Игроков: {player_n}/{_MAF_MIN_PLAYERS}+")
            return
        ...
        if data.startswith("maf_start_"):
            ...
            if g["creator"] != uid:
                bot.answer_callback_query(call.id, "Только создатель может начать!"); return
...
def _on_text_inner(msg):
    ...
    if tl in ("мафия выйти", "стоп мафия_REMOVED"):
        # уже обработано выше
        pass
...
def start_group_mafia(chat_id):
    ...
    if chat_id in _group_mafia and _group_mafia[chat_id].get("state") == "playing":
        ...
        return
    # Найдём первого реального пользователя из группы как создателя
    creator = 0
    for uid in _group_users.get(chat_id, set()):
        if uid > 0:
            creator = uid; break
    maf_open_group(chat_id, creator)
...
def _maf_send_all(lobby_id: int, text: str, kb=None):
    ...
    bots = g["bots"]
    if _maf_is_group(g):
        # В группе — отправляем в группу
        send_group(g["chat_id"], text, kb=kb)
    else:
...
def _maf_alive_text(g) -> str:
    bots = g["bots"]
    return "\n".join(
        f"  {'🤖' if uid in bots else '👤'} {g['player_names'].get(uid,'?')}"
        for uid in g["alive"]
    )
...
def _maf_lobby_text(lid: int) -> str:
    ...
    bots = g["bots"]
    names = "\n".join(
        f"  {'🤖' if p in bots else '👤'} {g['player_names'].get(p,'?')}"
        for p in g["players"]
    )
    need = max(0, _MAF_MIN_PLAYERS - len(g["players"]))
    need_str = f"\n⏳ Нужно ещё: {need}" if need > 0 else "\n✅ Достаточно игроков!"
    return (
        f"🔫 МАФИЯ — Лобби #{lid}\n\n"
        f"Игроков: {len(g['players'])}/{_MAF_MIN_PLAYERS}+{need_str}\n\n"
        f"Участники:\n{names}\n\n"
        f"⏱ Лобби ждёт 5 мин, затем ИИ-боты заполнят свободные места.\n"
        f"Или нажми «Старт» прямо сейчас."
    )
...
def maf_open_group(chat_id: int, creator_uid: int):
    ...
    _group_mafia[chat_id] = {"state": "lobby", "lid": lid}  # маппинг чата
...
def maf_begin(lobby_id: int):
    ...
    if cur < _MAF_MIN_PLAYERS:
        ...
        bots_note = f"🤖 Добавлено {needed} ИИ-бот(а): {', '.join(added)}\n\n"
    else:
        bots_note = ""
    ...
    start_text = (
        f"🎭 МАФИЯ НАЧИНАЕТСЯ!\n\n"
        f"{bots_note}"
        f"Игроков: {n} (реальных: {real_n}, ИИ: {bots_n})\n\n"
        f"{'💬 Общайтесь в чате! Ваши сообщения видят все живые игроки.' if not is_group else '💬 Общайтесь прямо в группе!'}\n"
    ...
    def _intro(_lid=lobby_id):
        ...
        intro = ask_ai(
            f"Ты зловещий ведущий игры Мафия. Игроки: {', '.join(names[:8])}. "
            f"Объяви начало игры жутко и атмосферно — 2-3 предложения. "
            f"Город погружается во мрак. Среди нас есть убийцы.",
...
def _maf_day(lobby_id: int):
    ...
    host = ask_ai(
        f"Ты ведущий Мафии. День {g['day_num']}. Живых игроков: {len(g['alive'])}. "
        f"Объяви начало дня — напряжённо и жутко. 1-2 предложения.",
    ...
    # ИИ-боты пишут обсуждение и голосуют
    def _bots_day(_lid=lobby_id):
        ...
        for bid in list(bots2):
            ...
            comment = ask_ai(
                f"Ты игрок '{bname}' в Мафии. Твоя роль — {brole} (секрет!). "
                f"День {g3['day_num']}. Напиши 1 короткую реплику: "
                f"подозревай кого-нибудь или защищайся. Как живой человек.",
                chat_id=_lid
            )
...
def _maf_chat_broadcast(lobby_id: int, sender_uid: int, text: str):
    ...
    icon = "🤖" if sender_uid in bots else "👤"
    out = f"{icon} {sender_name}: {text}"

    if _maf_is_group(g):
        # В группе уже видно — ничего не делаем (сообщение уже в чате)
        # Но для ботов-отправителей — пишем от имени
...
def _maf_check_votes(lobby_id: int):
    ...
    if not g or g["phase"] != "day":
        return
    bots = g["bots"]
    alive_real = [p for p in g["alive"] if p not in bots]
    # Ждём: все живые (боты голосуют сами через задержку)
    ...
    def _resolve(_lid=lobby_id):
        ...
        if not count:
            host = ask_ai(
                "Ты ведущий Мафии. Все воздержались — никого не устранили. "
                "Объяви коротко, напряжённо. 1 предложение.",
                chat_id=_lid, dm_mode=True
            )
            ...
            if len(top) > 1:
                ...
                host = ask_ai(
                    f"Ты ведущий Мафии. Ничья: {names_tie}. "
                    f"Объяви — никого не устраняют. 1 предложение.",
                    chat_id=_lid, dm_mode=True
                )
                ...
                host = ask_ai(
                    f"Ты ведущий Мафии. Устранён '{ename}'. "
                    f"Объяви жутко — НЕ НАЗЫВАЙ его роль до конца объявления. 1-2 предложения.",
                    chat_id=_lid, dm_mode=True
                )
...
def _maf_night(lobby_id: int):
    ...
    host = ask_ai(
        f"Ты ведущий Мафии. Ночь {g['day_num']}. Объяви ночь жутко. 1-2 предложения.",
        chat_id=lobby_id, dm_mode=True
    )
    night_text = (
        f"🌙 НОЧЬ {g['day_num']}\n\n"
    ...
    def _bots_night(_lid=lobby_id):
        ...
        for bid in list(g2["bots"]):
            ...
            if brole == "мирный":
                continue
            if brole in ("мафия", "маньяк"):
                targets = [p for p in g2["alive"] if p != bid and g2["roles"].get(p) not in ("мафия",)]
                if brole == "маньяк":
...
def _maf_check_night(lobby_id: int):
    ...
    if not set(night_roles).issubset(set(g["night_actions"].keys())):
        return
    ...
    def _morning(_lid=lobby_id):
        ...
        if victims:
            ...
            host = ask_ai(
                "Ты ведущий Мафии. Доктор спас игрока. Никто не погиб. 1 предложение.",
                chat_id=_lid, dm_mode=True
            )
            _maf_send_all(_lid, f"☀️ УТРО\n\n🎭 Ведущий: {host}\n\nНикто не погиб этой ночью.")
        else:
            host = ask_ai(
                "Ты ведущий Мафии. Тихая ночь — никто не погиб. 1 предложение жутко.",
                chat_id=_lid, dm_mode=True
            )
            _maf_send_all(_lid, f"☀️ УТРО\n\n🎭 Ведущий: {host}\n\nНикто не погиб.")
...
def _maf_end(lobby_id: int, winner: str):
    ...
    roles_all = "\n".join(
        f"  {'🤖' if uid in bots else '👤'} {g['player_names'].get(uid,'?')} — {role}"
        for uid, role in g["roles"].items()
    )
    win_text = {
    ...
    finale = ask_ai(
        f"Ты ведущий Мафии. Игра окончена — победили {winner}. "
        f"Произнеси финальный монолог, жутко и ёмко. 2-3 предложения.",
        chat_id=lobby_id, dm_mode=True
    )
    ...
    def _scare_loop(_uid=target_uid, _name=name):
        ...
        for prompt in openers:
            if not _ai_scare_active.get(_uid):
                return
            msg = ask_ai(prompt, chat_id=_uid, dm_mode=True)
