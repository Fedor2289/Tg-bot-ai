...
def KB_VIC():
    ...
    k.add(KeyboardButton("🤖 ИИ пишет за меня"),  KeyboardButton("🎬 Персональный сценарий"))
    k.add(KeyboardButton("🔄 Сбросить"))
...
# ══════════════════════════════════════════════════════════════
#  ДИНАМИЧЕСКИЕ ИИ-СЦЕНАРИИ
# ══════════════════════════════════════════════════════════════

def generate_personal_scenario(uid: int) -> str:
    """Генерирует персональный 5-шаговый сценарий через ИИ на основе профиля."""
    u = U(uid)
    name    = u.get("name", "жертва")
    city    = u.get("city", "неизвестном городе")
    fear    = u.get("fear", "темнота")
    job     = u.get("job", "")
    interests = ", ".join(u.get("interests", [])[:3]) or "нет данных"

    # Свежие новости из города
    news = get_city_news_cached(city)
    news_line = f"Свежие новости из {city}: {news}. " if news else ""

    prompt = (
        f"Ты — мастер психологического хоррора. Создай 5-шаговый сценарий пугания для человека:\n"
        f"Имя: {name}, Город: {city}, Страх: {fear}, Работа: {job}, Интересы: {interests}.\n"
        f"{news_line}"
        f"Сценарий должен использовать РЕАЛЬНЫЕ данные человека. Каждый шаг — отдельное жуткое сообщение.\n"
        f"Формат: шаг 1: [текст] | шаг 2: [текст] | ... | шаг 5: [текст]\n"
        f"Каждый шаг максимум 30 слов. Очень жутко и персонально."
    )
    raw = ask_ai(prompt, chat_id=uid, dm_mode=True)
    return raw

def run_personal_scenario(uid: int):
    """Запускает персональный сценарий."""
    def _run(_uid=uid):
        u = U(_uid)
        scenario_raw = generate_personal_scenario(_uid)
        # Парсим шаги
        import re
        steps = re.findall(r'шаг ?(\d+) ?:.+?(?=[|]|$)', scenario_raw[:1000], re.IGNORECASE)
        if not steps:
            steps = [s.strip() for s in scenario_raw.split('|') if s.strip()]
        if not steps:
            steps = [scenario_raw]
        for step in steps[:5]:
            step = step.strip()
            if step:
                send(_uid, step, kb=KB(u.get("stage", 0)))
...
def get_city_leaderboard(city: str) -> str:
    """Топ игроков из конкретного города."""
    if not city:
        return "❌ Город не указан."
    city_lower = city.lower()
    victims = [
        (uid, u) for uid, u in users.items()
        if not is_admin(uid) and (u.get("city") or "").lower() == city_lower
    ]
    if not victims:
        return f"👥 Никого из {city} ещё нет в базе."
    victims.sort(key=lambda x: x[1].get("score", 0), reverse=True)
    lines = []
    medals = ["🥇","🥈","🥉"]
    for i, (uid, u) in enumerate(victims[:10]):
    ...
    return f"🌆 РЕЙТИНГ {city.upper()}\n\n" + "\n".join(lines)


# ══════════════════════════════════════════════════════════════
#  РЕЖИМ ТИШИНЫ ("Притвориться мёртвым")
# ══════════════════════════════════════════════════════════════

_ghost_mode: dict = {}  # uid → until_time

def set_ghost_mode(uid: int, hours: int = 24):
    """Жертва уходит в режим тишины."""
    _ghost_mode[uid] = time.time() + hours * 3600
    U(uid)["muted"] = True
    # Объявляем в истории у всех кто знает этого пользователя
    return f"💀 {U(uid).get('name','???')} больше не отвечает. Его больше нет."

def is_ghost_mode(uid: int) -> bool:
    t = _ghost_mode.get(uid)
    if t and time.time() < t:
        return True
    if uid in _ghost_mode:
        _ghost_mode.pop(uid)
        U(uid)["muted"] = False  # автоматически выходим
    return False

# ══════════════════════════════════════════════════════════════
#  НОВОСТИ ИЗ ГОРОДА (для персонализации хоррора)
# ══════════════════════════════════════════════════════════════

def get_city_news(city: str, max_chars: int = 200) -> str:
    """Получает заголовок новости из города через RSS/поиск."""
    if not city:
        return ""
    try:
        import urllib.request
        import urllib.parse
        # Google News RSS для русских новостей
        ...
        # Берём первый заголовок
        import re
        titles = re.findall(r"<title><!\[CDATA\[(.+?)\]\]></title>", xml)
        titles = [t for t in titles if len(t) > 10 and "Google" not in t]
        if titles:
            return titles[0][:max_chars]
    except Exception:
        pass
    return ""

_city_news_cache: dict = {}   # city → (timestamp, news)
_CITY_NEWS_TTL = 1800  # 30 минут

def get_city_news_cached(city: str) -> str:
    """Кэшированный запрос новостей."""
    if not city:
        return ""
    now = time.time()
    cached = _city_news_cache.get(city)
    if cached and now - cached[0] < _CITY_NEWS_TTL:
        return cached[1]
    news = get_city_news(city)
    _city_news_cache[city] = (now, news)
    return news
...
FEAR_SHOP = {
    "shield_1h":    {"name": "🛡 Щит (1 час)",         "desc": "Защита от хоррора на 1 час",          "price": 50},
    "shield_24h":   {"name": "🛡 Щит (сутки)",         "desc": "Защита от хоррора на 24 часа",        "price": 150},
    "hint_quest":   {"name": "💡 Подсказка в квесте",  "desc": "Подсказка в следующем задании",       "price": 30},
    "boost_fear":   {"name": "😈 Усилить страх",       "desc": "Отправить ужас другому (нужен ID)",   "price": 40},
    "silence_2h":   {"name": "🔕 Тишина (2 часа)",     "desc": "Бот временно замолчит",               "price": 35},
    "extra_daily":  {"name": "📋 Доп. задание",        "desc": "Ещё одно задание сегодня",             "price": 20},
}

_shop_shields: dict = {}   # uid → unshield_time
_shop_hints: set   = set() # uid — есть подсказка
_shop_silence: dict = {}   # uid → unsilence_time

def shop_buy(uid: int, item_id: str, target_uid: int = 0) -> tuple:
    """Покупка в магазине. Возвращает (ok, message)."""
    u = U(uid)
    item = FEAR_SHOP.get(item_id)
    if not item:
        return False, "❌ Товар не найден."
    price = item["price"]
    if u.get("score", 0) < price:
        return False, f"❌ Недостаточно очков. Нужно {price}, у тебя {u.get('score',0)}."
    u["score"] -= price
    now = time.time()

    if item_id == "shield_1h":
        _shop_shields[uid] = now + 3600
        return True, f"✅ Куплено: {item['name']}\nАктивно 1 час."
    elif item_id == "shield_24h":
        _shop_shields[uid] = now + 86400
        return True, f"✅ Куплено: {item['name']}\nАктивно 24 часа."
    elif item_id == "hint_quest":
        _shop_hints.add(uid)
        return True, f"✅ Куплено: {item['name']}\nПодсказка доступна при следующем задании."
    elif item_id == "boost_fear":
        if target_uid and target_uid in users and not is_admin(target_uid):
            _pool.submit(horror_tick, target_uid)
            return True, f"✅ Ужас отправлен жертве {users[target_uid].get('name','?')}. 😈"
        return False, "❌ Укажи ID жертвы."
    elif item_id == "silence_2h":
        _shop_silence[uid] = now + 7200
        return True, f"✅ Куплено: {item['name']}\nТишина на 2 часа."
    elif item_id == "extra_daily":
        _daily_done.pop(uid, None)  # сброс лимита дня
        return True, f"✅ Куплено: {item['name']}\nМожешь выполнить ещё одно задание сегодня."
    return False, "❌ Ошибка."

def is_shop_shielded(uid: int) -> bool:
    """Проверяет активен ли щит."""
    t = _shop_shields.get(uid)
    if t and time.time() < t:
        return True
    _shop_shields.pop(uid, None)
    return False

def is_shop_silenced(uid: int) -> bool:
    """Проверяет активна ли тишина."""
    t = _shop_silence.get(uid)
    if t and time.time() < t:
        return True
    _shop_silence.pop(uid, None)
    return False
...
def get_shop_text(uid: int) -> str:
    ...
    for iid, item in FEAR_SHOP.items():
        ...
        lines.append(f"{can} [{iid}] {item['name']} — {item['price']} очков\n   {item['desc']}")
...
ACHIEVEMENTS = {
    ...
    "scream_x10":     {"name": "😱 10 скримеров",       "desc": "Пережить 10 скримеров",               "reward": 30},
    "secrets_told":   {"name": "🤫 Открытая книга",     "desc": "Рассказать имя, возраст, город, страх","reward": 40},
...
def check_achievement(uid: int, achievement_id: str):
    """Выдаёт ачивку если ещё нет. Отправляет поздравление."""
    u = U(uid)
    earned = u.setdefault("achievements", [])
    ...
    u["score"] = u.get("score", 0) + a["reward"]
    def _notify(_uid=uid, _a=a):
        time.sleep(random.uniform(0.5, 2))
        send(_uid,
            f"🏅 ДОСТИЖЕНИЕ РАЗБЛОКИРОВАНО!\n\n"
            f"{a['name']}\n{a['desc']}\n\n"
            f"🏆 +{a['reward']} очков страха",
            kb=KB(U(_uid).get("stage", 0)))
    _pool.submit(_notify)
    return True

def get_achievements_text(uid: int) -> str:
    """Показывает ачивки пользователя."""
    u = U(uid)
    ...
    for aid, a in ACHIEVEMENTS.items():
        if aid in earned:
            ...
            lines.append(f"🔒 {a['name']} — {a['desc']}")
    total = len(earned)
    return f"🏅 АЧИВКИ ({total}/{len(ACHIEVEMENTS)})\n\n" + "\n".join(lines)
...
def _random_event_loop():
    """Фоновый цикл: каждые 2 часа шлёт случайное событие активным жертвам (stage>=2)."""
    _shutdown.wait(600)  # первый запуск через 10 минут после старта
    while not _shutdown.is_set():
        try:
            active = [
                uid for uid, u in list(users.items())
                if u.get("horror_active") and not u.get("stopped") and not u.get("muted")
                and u.get("stage", 0) >= 2 and not is_admin(uid)
            ]
            if active:
                # Выбираем случайного активного пользователя
                uid = random.choice(active)
                _pool.submit(horror_tick, uid)
        except Exception:
            pass
        _shutdown.wait(_RAND_EVENT_INTERVAL)
...
def horror_tick(uid):
    ...
    if u.get("stopped") or u.get("muted"):
        return
    # Щит из магазина
    if is_shop_shielded(uid) or is_ghost_mode(uid):
        return
...
def proc_game(uid, text):
    ...
    # Кнопки главного меню — выходим из игры и передаём управление
    if text in _MAIN_BUTTONS:
        del games[uid]; return False
...
def _handle_admin_inner(msg, admin_uid):
        ...
        if text == "👥 Группы (управление)":
            ...
            for cid in _group_users.keys():
                ...
                kb_groups.add(
                    ...
                    InlineKeyboardButton(f"🤖 ИИ в группу {cid}",   callback_data=f"gadm_aiwrite_{cid}"),
    ...
    # ── 🤖 ИИ пишет за меня ─────────────────────────────────────
    if text == "🎬 Персональный сценарий":
        tid = adm_state.get(admin_uid, {}).get("target_uid")
        if not tid or tid not in users:
            send(admin_uid, "⚠️ Сначала выбери жертву (⚙️ Выбрать жертву).", kb=adm_kb(admin_uid)); return
        send(admin_uid, f"🎬 Генерирую персональный сценарий для {users[tid].get('name','?')}...", kb=adm_kb(admin_uid))
        def _gen_scen(_tid=tid, _aid=admin_uid):
            run_personal_scenario(_tid)
            send(_aid, f"✅ Сценарий запущен!", kb=adm_kb(_aid))
        _pool.submit(_gen_scen)
        return

    if text == "🤖 ИИ пишет за меня":
        tid = adm_state.get(admin_uid, {}).get("target_uid")
        if not tid or tid not in users:
            send(admin_uid, "⚠️ Сначала выбери жертву (⚙️ Выбрать жертву).", kb=adm_kb(admin_uid))
            return
        u_t = users[tid]
        # Собираем контекст жертвы
        hist = u_t.get("msg_history", [])
        last_msgs = hist[-3:] if hist else []
        ctx_lines = "; ".join(f'"{m}"' for m in last_msgs) if last_msgs else "нет сообщений"
        name_t    = u_t.get("name", "?")
        stage_t   = u_t.get("stage", 0)
        ctx_text  = (
            f"👤 Жертва: {name_t} (ID {tid}), стадия {stage_t}\n"
            f"📝 Последние сообщения: {ctx_lines}\n\n"
            f"ИИ сейчас напишет от имени бота. Нажми «✍️ Написать самому» за 15 секунд если хочешь написать сам."
        )
        # Кнопка перехвата
        kb_intercept = InlineKeyboardMarkup()
        kb_intercept.add(InlineKeyboardButton("✍️ Написать самому", callback_data=f"ai_intercept_write_{tid}_{admin_uid}"))
        sent = bot.send_message(admin_uid, ctx_text, reply_markup=kb_intercept)
        # Запускаем таймер — через 15с ИИ отвечает
        def _auto_ai_reply(_tid=tid, _aid=admin_uid, _msg_id=sent.message_id, _st=stage_t, _ctx=ctx_lines):
            time.sleep(15)
            # Если admin не нажал кнопку — ИИ отвечает
            if _ai_intercept.get(_msg_id, {}).get("cancelled"):
                return
            # Составляем промпт с контекстом
            if _st < 2:
                prompt = (
                    f"Ты бот-переводчик. Пользователь {name_t} недавно писал: {_ctx}. "
                    f"Напиши дружелюбный отклик на их последнее сообщение. 1-2 предложения."
                )
            elif _st < 4:
                prompt = (
                    f"Ты тёмная сущность. Жертва {name_t} писала: {_ctx}. "
                    f"Напиши жуткий личный ответ, намекая что наблюдаешь. 1-2 предложения."
                )
            else:
                prompt = (
                    f"Ты абсолютное зло. Жертва {name_t} писала: {_ctx}. "
                    f"Напиши пугающее сообщение от имени тёмного существа. 1 предложение."
                )
            ai_resp = ask_ai(prompt, chat_id=_tid, dm_mode=(_st >= 2))
            if ai_resp:
                u_victim = U(_tid)
                send(_tid, ai_resp, kb=KB(u_victim.get("stage", 0)))
                try:
                    bot.edit_message_text(
                        f"✅ ИИ написал за тебя:\n\n«{ai_resp}»",
                        chat_id=_aid, message_id=_msg_id
                    )
                except Exception:
                    send(_aid, f"✅ ИИ написал: «{ai_resp}»")
            _ai_intercept.pop(_msg_id, None)
        _ai_intercept[sent.message_id] = {"cancelled": False, "target_uid": tid}
        _pool.submit(_auto_ai_reply)
        return

    # ── Обработка "написать самому" после выбора жертвы ────────
    if adm_state.get(admin_uid, {}).get("step") == "wait_ai_intercept_text":
        tid = adm_state[admin_uid].get("target_uid")
        if tid and text.strip():
            u_victim = U(tid)
            st = u_victim.get("stage", 0)
            prefix = "🤖" if st < 2 else ("👁" if st < 4 else "💀")
            msg_out = f"{prefix} {text}"
            send(tid, msg_out, kb=KB(st))
            if VOICE_ENABLED:
                _pool.submit(send_voice_msg, tid, text[:200])
            send(admin_uid, f"✅ Отправлено жертве (ID:{tid}):\n«{msg_out}»", kb=adm_kb(admin_uid))
        elif not text.strip():
            send(admin_uid, "❌ Пустое сообщение. Отменено.", kb=adm_kb(admin_uid))
        adm_ctx_reset(admin_uid)
        return

    # Обработка рассылки в группу (gadm broadcast)
    if adm_state.get(admin_uid, {}).get("step") == "wait_grp_aiwrite":
        cid = adm_state[admin_uid].get("grp_cid")
        if cid and text.strip():
            send_group(cid, f"🤖 {text}")
            if GROUP_AUTO_VOICE:
                _pool.submit(_send_group_voice, cid, text[:200])
            send(admin_uid, f"✅ Отправлено в группу {cid}: «{text}»", kb=adm_kb(admin_uid))
        elif not text.strip():
            send(admin_uid, "❌ Пустое сообщение.", kb=adm_kb(admin_uid))
        adm_ctx_reset(admin_uid); return
...
def on_start(msg):
    ...
    def _do_start(_uid=uid, _uname=uname):
        ...
        _card_story.pop(_uid, None)
        # Выход из мафии если был
        try:
            _lid_s = _maf_uid.pop(_uid, None)
            if _lid_s:
                _gm = _maf.get(_lid_s)
                if _gm:
                    try: _gm["alive"].remove(_uid)
                    except: pass
                    try: _gm["players"].remove(_uid)
                    except: pass
        except Exception:
            pass
...
def on_callback(call):
    ...
    try:
        ...
        if data.startswith("gadm_") and is_admin(uid):
            # Handle manual write after AI intercept: gadm_aiwrite_manual_{cid}_{uid}
            if data.startswith("gadm_aiwrite_manual_"):
                sub = data[len("gadm_aiwrite_manual_"):]
                cid_s, uid_s = sub.rsplit("_", 1)
                _cid2 = int(cid_s); _uid2 = int(uid_s)
                for k in list(_ai_intercept.keys()):
                    _ai_intercept[k]["cancelled"] = True
                bot.answer_callback_query(call.id, "✍️ Напиши сообщение")
                adm_ctx_reset(_uid2)
                adm_state[_uid2] = {"step": "wait_grp_aiwrite", "grp_cid": _cid2}
                try: bot.edit_message_reply_markup(chat_id=_uid2, message_id=call.message.message_id, reply_markup=None)
                except: pass
                send(_uid2, f"✍️ Напиши сообщение для группы {_cid2}:")
                return
            parts = data.split("_")
            action = parts[1]; cid = int(parts[2])
            bot.answer_callback_query(call.id)
            if action == "horror":
                ...
                # ИИ пишет в группу (с перехватом)
                if not AI_ENABLED:
                    bot.answer_callback_query(call.id, "❌ ИИ недоступен"); return
                # Собираем контекст последних сообщений в группе
                members = _group_users.get(cid, set())
                recent = []
                for mid in members:
                    h = users.get(mid, {}).get("msg_history", [])
                    if h: recent.append(f"{users[mid].get('name','?')}: {h[-1]}")
                ctx = "; ".join(recent[-5:]) if recent else "тихо в группе"
                preview_txt = (
                    f"🤖 ИИ напишет в группу {cid}\n\n"
                    f"Контекст: {ctx[:200]}\n\n"
                    f"Нажми «✍️ Написать самому» за 15с — иначе ИИ напишет сам."
                )
                kb_gi = InlineKeyboardMarkup()
                kb_gi.add(InlineKeyboardButton("✍️ Написать самому", callback_data=f"gadm_aiwrite_manual_{cid}_{uid}"))
                sent = bot.send_message(uid, preview_txt, reply_markup=kb_gi)
                def _grp_auto_ai(_cid=cid, _uid=uid, _mid=sent.message_id, _ctx=ctx):
                    time.sleep(15)
                    if _ai_intercept.get(_mid, {}).get("cancelled"): return
                    prompt = f"Ты загадочный бот в Telegram-группе. Напиши что-нибудь интересное или провокационное в чат. Контекст: {_ctx[:150]}. 1-2 предложения по-русски."
                    resp_ai = ask_ai(prompt, chat_id=_cid)
                    if resp_ai:
                        send_group(_cid, f"🤖 {resp_ai}")
                        try: bot.edit_message_text(f"✅ ИИ написал в группу:\n«{resp_ai}»", chat_id=_uid, message_id=_mid)
                        except: pass
                    _ai_intercept.pop(_mid, None)
                _ai_intercept[sent.message_id] = {"cancelled": False}
                _pool.submit(_grp_auto_ai)
            return

        # ── Кнопка "✍️ Ответить за ИИ" (новый перехват) ────────
        if data.startswith("ai_ic_"):
            # data = "ai_ic_{key}_{admin_uid}"
            try:
                aid = int(data.split("_")[-1])
                intercept_key = "_".join(data.split("_")[2:-1])
            except Exception:
            ...
            if ic.get("group"):
                ...
                adm_state[aid] = {"step": "wait_grp_aiwrite", "grp_cid": grp_cid}
                send(aid, f"✍️ Пиши ответ для группы {grp_cid}. Отправлю как 🤖 ИИ:")
            else:
                target_uid = ic.get("uid")
                adm_state[aid] = {"step": "wait_ai_intercept_text", "target_uid": target_uid}
                u_tgt = U(target_uid)
                send(aid, f"✍️ Пиши ответ для {u_tgt.get('name', target_uid)}. Отправлю от имени ИИ:")
            return

        # ── Старый callback "✍️ Написать самому" (обратная совместимость) ──
        if data.startswith("ai_intercept_write_"):
            parts = data.split("_")
            try:
                tid = int(parts[3]); aid = int(parts[4])
            except Exception:
                bot.answer_callback_query(call.id, "Ошибка"); return
            for k in list(_ai_intercept.keys()):
                if _ai_intercept[k].get("uid") == tid or _ai_intercept[k].get("target_uid") == tid:
                    _ai_intercept[k]["cancelled"] = True
            bot.answer_callback_query(call.id, "✍️ Напиши сообщение")
            adm_ctx_reset(aid)
            adm_state[aid] = {"step": "wait_ai_intercept_text", "target_uid": tid}
            try:
                bot.edit_message_reply_markup(chat_id=aid, message_id=call.message.message_id, reply_markup=None)
            except Exception:
                pass
            send(aid, f"✍️ Напиши сообщение для жертвы {tid}:")
            return
...
def _on_text_inner(msg):
    ...
    # ── Режим ИИ работает и для админов (ДО редиректа в панель) ─
    ...
    if _hour == 3:
        check_achievement(uid, "night_owl")
    if stage >= 5:
        check_achievement(uid, "stage_max")
    ai_msgs = u.get("ai_msg_count", 0)
    if ai_msgs >= 50:
        check_achievement(uid, "ai_chat_50")
    ...
    if u.get("ai_mode") and text and not text.startswith("/"):
        ...
        ui_buttons = {
            ...
            "🔫 Мафия","🔫 Мафия (создать лобби)","🗡 Мини-RPG","📖 Страшные истории",
            "🔦 Квест-головоломка","🎭 Карточная история","🗓 Задание дня","🏆 Мой рейтинг",
        }
        if text not in ui_buttons:
            ...
            def _ai_dialog(_q=question, _uid=uid, _stage=stage, _u=u, _kb=kb):
                """AI dialog с перехватом для админа."""
                _name = _u.get("name") or "ты"
                _city = _u.get("city") or "твоём городе"
                _fear = _u.get("fear") or "темнота"
                _prefix = "🤖" if _stage < 2 else ("👁" if _stage < 4 else "💀")

                if _stage < 2:
                    _prompt = (f"Пользователь спрашивает: '{_q}'. Ты — умный помощник. "
                               f"Ответь полезно по-русски. Максимум 3 предложения.")
                elif _stage < 4:
                    _prompt = (f"Жертва {_name} спрашивает: '{_q}'. Ты — тёмная сущность. "
                               f"Ответь по существу, но жутко. 2-3 предложения.")
                elif _stage < 6:
                    if random.random() < 0.6:
                        _prompt = (f"Жертва {_name} (страх: {_fear}) спрашивает: '{_q}'. "
                                   f"Ты — абсолютное зло. Ответь искажённо. 2 предложения.")
                    else:
                        _prompt = (f"Жертва пытается говорить. Вопрос: '{_q}'. "
                                   f"Ты — зло. Пугай. 1-2 предложения.")
                else:
    ...
    # ── Команды магазина / ачивок ──────────────────────────────
    if tl.startswith("купить ") or tl.startswith("buy "):
        item_id = text.split(None, 1)[1].strip() if " " in text else ""
        # Для "усилить страх" может быть ID цели
        parts = item_id.split()
        target = 0
        if len(parts) >= 2 and parts[-1].isdigit():
            target = int(parts[-1])
            item_id = parts[0]
        ok, msg = shop_buy(uid, item_id, target)
        send(uid, msg, kb=kb); return

    if tl in ("магазин", "shop", "🛒 магазин"):
        send(uid, get_shop_text(uid), kb=kb); return

    if tl in ("ачивки", "достижения", "achievements"):
        send(uid, get_achievements_text(uid), kb=kb); return

    if tl in ("рейтинг города", "топ города") and u.get("city"):
        send(uid, get_city_leaderboard(u["city"]), kb=kb); return

    if tl in ("тишина", "ghost", "не беспокоить"):
        msg_ghost = set_ghost_mode(uid, hours=24)
        send(uid, f"💀 Режим тишины активирован на 24 часа.\nПиши «вернуться» чтобы выйти.", kb=kb)
        return

    if tl in ("вернуться", "я здесь", "return"):
        if is_ghost_mode(uid):
            _ghost_mode.pop(uid, None)
            U(uid)["muted"] = False
...
def _handle_group_message(msg, uid, chat_id, text):
    ...
    if text == "🤖 Спросить ИИ":
        if not AI_ENABLED:
            ...
            _group_awaiting[chat_id] = ("ai", uid)
            send_group(chat_id, f"🤖 {uname}, спрашивай — отвечу.")
        return
...
_ai_hist_time: dict = {}   # chat_id → last_access timestamp
_AI_HIST_MAX    = 20   # максимум сообщений в истории
_AI_HIST_TTL    = 3600  # TTL истории — 1 час неактивности
...
def ask_ai(prompt: str, chat_id: int = 0, dm_mode: bool = False) -> str:
    ...
    try:
        with _ai_lock:
            now_t = time.time()
            # TTL: сбрасываем историю если давно не общались
            if chat_id in _ai_hist_time and now_t - _ai_hist_time[chat_id] > _AI_HIST_TTL:
                _ai_history.pop(chat_id, None)
            _ai_hist_time[chat_id] = now_t
            hist = _ai_history.setdefault(chat_id, [])
            hist.append({"role": "user", "content": prompt})
            if len(hist) > _AI_HIST_MAX:
                hist[:] = hist[-_AI_HIST_MAX:]
        ...
        if _ai_backend == "groq":
            ...
            for _gm in _groq_models:
                try:
                    ...
                    continue
            if resp is None:
                raise Exception(f"Groq all models failed")
        else:  # cerebras
            # Пробуем мощную модель, фоллбэк на быструю
            _cer_models = ["llama-3.3-70b", "llama3.3-70b", "llama3.1-8b"]
            resp = None
            last_cer_err = None
            for _m in _cer_models:
                try:
                    resp = _ai_client.chat.completions.create(
                        model=_m,
                        messages=messages,
                    ...
                    continue
            if resp is None:
                raise Exception(f"Cerebras all models failed: {last_cer_err}")
...
def ask_ai_host(prompt: str, chat_id: int = 0) -> str:
    ...
    try:
        ...
        if _ai_backend == "groq":
            ...
            _cer_m_list = ["llama-3.3-70b", "llama3.3-70b", "llama3.1-8b"]
            resp = None
            for _cm in _cer_m_list:
                try:
                    resp = _ai_client.chat.completions.create(
                        model=_cm,
                        messages=messages,
                    ...
                    continue
            if resp is None:
                raise Exception("Cerebras host: no working model")
...
if __name__ == "__main__":
    ...
    print("╔══════════════════════════════════════════╗")
    print("║     👁  HORROR BOT v21.0 ЗАПУЩЕН  👁     ║")
    print("╠══════════════════════════════════════════╣")
